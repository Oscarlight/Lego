...
mat Device1D::mobileEDensityArrayFunct(mat phin) {
    ...
    for ( int i=0; i < materialList.size(); i++) {
        for ( int j=0; j < nyList[i]; j++) {
            if (typeList[i] == Semiconductor) {
                density(k) = materialList[i].mobileElectronDensity( (double)phin(k) ); // why here used to be k+1, then "carrierDensity[0]=carrierDensity[1];"
...
//                std::cout << (double)density(k) << std::endl;
            	...
            	density(k) = materialList[i].fixChargeDensity();
             };
            k++;
        };
    };
